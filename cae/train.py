...
def main():
    ...
    handle_coding = model.t_conv0.register_forward_hook(get_activation(activations, 'coding'))
    


    # activations_path = os.path.join(os.path.dirname(activations_path), 'activations')


    # Run model for one epoch (I use the validation function but for train_loader)
    valid(model, criterion, train_loader, device, epoch)
    ...
    for name, outputs in activations.items():
        if outputs:
            file_path = os.path.join(activations_path ,name + '_' + checkpoint['name'][:-3] + '.pkl')
            print(file_path)
            with open(file_path,'wb') as file: 
                np.save(file, torch.cat(outputs, 0).cpu().numpy())
                activations[name] = []
